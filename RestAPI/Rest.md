...
| Header        | Example                                                                                  | Possible Values                                 | Description                                                                                                |
| ------------- | ---------------------------------------------------------------------------------------- | ----------------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| Accept        | Accept: text/html                                                                        | text/html, application/json, _/_                | Indicates the media types that are acceptable for the response.                                            |
...
| Content-Security-Policy (prevents XSS attacks) (see below) | Content-Security-Policy: default-src 'self'; script-src 'self' https://trusted-scripts.com; (indicates scripts can be loaded only from this domain) | scrip-src- (controls which scripts browsers can load)      | Controls the resources the browser is allowed to load for a given page. | Preventing cross-site scripting (XSS) attacks by limiting resource loading.    |
...
| ETag (express automatically sets this for us for static contents) | ETag: "33a64df551425fcc55e4d42a148795d9f25f89d4" | String (e.g., hash or version identifier)                      | server sends a hased string (ETAG) for static files, When a client makes a request with the ETag, the server can respond with 304 Not Modified if the resource hasn't changed, avoiding the need to send the full response again. | Ensuring efficient caching by validating whether the cached version is still fresh.                                                                                      |
