...
5. **process.nextTick()** - tick - when the event loop iterates over all of phases for one time, use - assign event handlers after an object has been constructed but before any I/O has occurred, setImmediate vs settimeout - which runs frst? depends on current pos. of evnt loop
...
6. **Streams** - collection of data rhat does not fit in memory.
7. **Scaling node js app** - cluster module,
