...
5. **process.nextTick()** - tick - when the event loop iterates over all of phases for one time, use - assign event handlers after an obj. has been constructed but before any I/O has occurred, setImmediate vs settimeout-which runs frst? depends on curr. pos. of evnt loop
