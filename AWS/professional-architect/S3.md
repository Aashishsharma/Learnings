...
# IMP - the sync can be in reverse direction as well
# so we can dpwnload entire s3 bucket into local directory by 
# changing the source and des placeholder in above sync command
```
...
- now, we can run Athena on this target bucket to analyze the logs

## S3 Event Notifications

Your overall understanding is **correct**, but a few points need correction.

- Allows S3 to send notifications when **specific events occur on objects in a bucket**.
...
- **SQS FIFO is not directly supported** as an S3 Event Notification destination.

## S3 storage class analysis
- analyzes storage access patterns of objects, and it will **recommend to move objects from Standard to StandardIA**
- need to enable this for atleast 30 days to analyze the pattern
- but Intelligent Tiering already does this
- Intelligent tiering is to move objects across all storage classes, this one is only to move from Standard to standard IA
- this one does not use ML and it is cheaper

## S3 storage lens
![alt text](../PNG/ps33.PNG "Title")  

## S3 Multipart Upload


Used to upload **large objects in multiple parts**, potentially in parallel.
...
2. **UploadPart**
   - Upload each part using the `UploadId`.
   - For each part we assign a **PartNumber**.
   - Parts can be uploaded **in parallel**.
   - For each upload, s3 will return ETag
...
3. **CompleteMultipartUpload**
   - Provide the `UploadId` + list of uploaded parts/ETags, we need to pass Etags, so s3 can finzalize the ordering of final object
   - S3 combines the parts into the final object.
   ```bash
...
- **Key points**  
1. We can upload the parts any time, maybe after 15 days, it is fine
2. On our side, we need to split the object, we can do it with some tools, or use the split command in linux
3. in s3, we will see the final object, we won't see the parts

## S3 byte range fetching
- allows us to fetch range of bytes of an object
- like we can ask s3 to send only first 100 bytes from a 100GB object
- obviously getting 100bytes might not make sense, but we can do multiple byte range fetching in paralle to boose performance

![alt text](../PNG/ps34.PNG "Title")  
