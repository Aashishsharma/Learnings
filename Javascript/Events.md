...
7. **Mutation observer** - built-in obj, observes a DOM element and fires a callback when it detects a change, after React this is rarely used, because react has state, observer = new MutationObserver(cb); observer.observe(node, config) config('changes to watch' - (childList, subtree, attributes))
