...
7. **Mutation observer** - built-in obj, observes a DOM element and fires a callback when it detects a change, after React (due to state) this is rarely used, observ = new MutationObserver(cb); observe.observe(node, conf) conf('changes to watch'(childList, subtree, attributes))
