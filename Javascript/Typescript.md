...
const greetGuitarist = (guitarist: Guitarist) => {
  if (guitarist.name) {
    // this is called typeguard or narrowing -- see below explaination
...
// if instead of never if we used _exhaustiveCheck: string = animal, TS won't comaplin if we add new union type to animal,
// but since we added never - it tell TS that default case has run but it should never be run beacuse type is never

// think of never as dead code fallback, if never is returned, that means that code should be un-reachable
```
...
```typescript
// e.g.
const htmlElem = document.getElementByID("myCanvas")
  .innerHTML as HTMLCanvasElement;
...
type StockStatus<T> = {
  [K in keyof T]: T[K] extends { stockQuantity: 0 }
    ? "out of stock"
    : "available";
};
...
const productStatus: ProductStatus = products.reduce(
